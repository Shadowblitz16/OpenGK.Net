...
{
    public static readonly int Count = (int)Enum.GetValues<KeyCode>().Max()+1;
    private static bool[] pressed  = new bool[Count];
    private static bool[] released = new bool[Count];

    internal static unsafe void KeyCallback(GLFWWindow* window, Keys key, int scanCode, InputAction action, KeyModifiers mods)
    {
        if      (action == InputAction.Press  )
        {
            var index = ((int)key);
            if (Enum.IsDefined<Keys>((Keys)index)) 
            {
                pressed [index] = true;
    ...
    public static bool IsPressed  (KeyCode key)
    {
        ...
        if (Enum.IsDefined<Keys>((Keys)index)) return  pressed[index] &&  released[index];
        return false;    
    }
    ...
    public static bool WasReleased(KeyCode key)
    {
        var index = ((int)key);
        var code  = (KeyCode)index;
        if (code == KeyCode.Any)
